...
class TrainValDataset(BaseAnomalyDetectionDataset):
    def __init__(self, split, class_name, img_size, dataset_path, few_shot = None, few_shot_seed = 42):
        super().__init__(split = split, class_name = class_name, img_size = img_size, dataset_path = dataset_path)

        self.img_paths, self.labels = self.load_dataset()  # self.labels => good : 0, anomaly : 1
        if few_shot is not None:
            if few_shot <= 0:
                raise ValueError("few_shot must be a positive integer.")
            total_samples = len(self.img_paths)
            num_samples = min(few_shot, total_samples)
...
def get_data_loader(split, class_name, dataset_path, img_size = 224, batch_size = 1, shuffle = False, few_shot = None, few_shot_seed = 42):
    if split in ['train']:
        dataset = TrainValDataset(split = "train", class_name = class_name, img_size = img_size, dataset_path = dataset_path, few_shot = few_shot, few_shot_seed = few_shot_seed)
