...
class FeatureExtractors(torch.nn.Module):
    def __init__(self, device, 
        ...
        feature_weights_dir = feature_weights_dir or os.environ.get("FEATURE_EXTRACTOR_WEIGHTS", "/path/to/your/FeatureExtractorWeights")
        dino_weight_path = os.path.join(feature_weights_dir, "dino_vitbase8_pretrain.pth")
        pointmae_weight_path = os.path.join(feature_weights_dir, "pointmae_pretrain.pth")
        self.rgb_backbone.load_state_dict(torch.load(dino_weight_path), strict=True)
        ...
        self.xyz_backbone.load_model_from_ckpt(pointmae_weight_path)
        # ! Use only the first k blocks.
